 ...
 *   01:00–07:00  → каждые 90 мин (ночь)
 *   07:00–09:00  → каждые 15 мин (утренний пик)
 *   18:00–22:00  → каждые 15 мин (вечерний пик)
...
function getIntervalMs() {
    const h = getMskHour();
    if (h >= 1 && h < 7)  return 90 * 60 * 1000; // ночь — 90 мин
    if ((h >= 7 && h < 9) || (h >= 18 && h < 22)) return 15 * 60 * 1000; // пик — 15 мин
    return 30 * 60 * 1000; // стандарт — 30 мин
}

function getIntervalLabel() {
    const h = getMskHour();
    if (h >= 1 && h < 7)  return '90 мин (ночь)';
    if ((h >= 7 && h < 9) || (h >= 18 && h < 22)) return '15 мин (пик)';
    return '30 мин';
}
...
bot.onText(/\/start/, msg => {
    ...
    bot.sendMessage(msg.chat.id,
        ...
        `   • 01:00–07:00 → каждые *90 мин* 🌙\n` +
        `   • 07:00–09:00 → каждые *15 мин* ☀️\n` +
...
console.log(`⏱ 01-07 МСК: 90 мин | 07-09 и 18-22: 15 мин | остальное: 30 мин`);
